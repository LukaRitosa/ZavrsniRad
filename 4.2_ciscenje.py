...
df = pd.read_csv("data2/tools_features3.csv")

# Samo Virtual Classroom
df = df[df["category"] == "Virtual Classroom"].copy()

cols_osnovni = ["name", "slug", "category", "rating"]

# Uzmi samo feature stupce
feature_cols = df.columns[5:]

# Samo featurei koje imaju svi alati
cols_features = [
...
if not cols_features:
    ...
    print(
        f"⚠️ Nema featurea koje imaju svi alati."
    )
    print(
        f"Koristim {len(cols_features)} featurea gdje barem jedan alat ima vrijednost."
    )
else:
    print(
        f"✓ Pronađeno {len(cols_features)} zajedničkih featurea."
    )

df_out = df[cols_osnovni + cols_features]
...
print("\nSpremljeno u data2/features_virtual_classroom3.csv")


'''
⚠️ Nema featurea koje imaju svi alati.
Koristim 239 featurea gdje barem jedan alat ima vrijednost.
